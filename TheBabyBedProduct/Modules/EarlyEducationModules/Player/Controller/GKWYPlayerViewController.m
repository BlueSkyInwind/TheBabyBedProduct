...
//    self.gk_navBackgroundColor = [UIColor clearColor];
    ...
    self.titleView.frame = CGRectMake(0, 0, SCREEN_WIDTH - 160.0f, 44.0f);
//    self.gk_navTitleView = self.titleView;
    [self.view addSubview:self.titleView];
    
    self.nameLabel                  = [CBAutoScrollLabel new];
    self.nameLabel.frame            = CGRectMake(0, 0, SCREEN_WIDTH - 160.0f, 20);
    ...
    self.artistLabel.top       = self.nameLabel.bottom + 2;
}
...
//            NSLog(@"获取详情失败==%@", error);
//            [QMUITips showError:@"数据请求失败，请检查网络后重试！"];
//        }];
    }
}
...
- (void)setupTitleWithModel:(GKWYMusicModel *)model {
    ...
    self.nameLabel.centerX = self.titleView.width * 0.5;
    
    [self.artistLabel sizeToFit];
    self.artistLabel.top       = self.nameLabel.bottom + 2;
    self.artistLabel.centerX = self.titleView.width * 0.5;
}
...
- (void)controlView:(GKWYMusicControlView *)controlView didClickList:(UIButton *)listBtn {
    ...
    self.listView.size = CGSizeMake(self.view.width, 440);
