...
- (instancetype)init {
    if (self = [super init]) {
        ...
        [self.diskBgView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.width.height.mas_equalTo(SCREEN_WIDTH - 75.0f);
        }];
...
- (void)layoutSubviews {
    ...
    self.needleView.centerX = SCREEN_WIDTH * 0.5 + 25.0f;
    self.needleView.top       = -25.0f;
...
- (UIView *)diskBgView {
    if (!_diskBgView) {
        ...
        _diskBgView.layer.cornerRadius = (SCREEN_WIDTH - 75.0f) * 0.5;
    }
    return _diskBgView;
}
...
- (UIScrollView *)diskScrollView {
    if (!_diskScrollView) {
        ...
        _diskScrollView.contentSize = CGSizeMake(SCREEN_WIDTH * 3, 0);
        _diskScrollView.scrollEnabled = NO;
    }
