...
{
    if (self.ppTextfieldStartEditBlock) {
        self.ppTextfieldStartEditBlock(self);
    }
}
...
//            JRLog(@"点击过快qian--%ld---%@---%@",_maxTextLength,toBeString,self.text);
            if (toBeString.length > _maxTextLength) {
                self.text = [toBeString substringToIndex:_maxTextLength];
            }
//            JRLog(@"点击过快--%ld---%@---%@",_maxTextLength,toBeString,self.text);
